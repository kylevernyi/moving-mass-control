...
Vector3d ConvertMotorSpeedToMassVelocity(int32_t xdot, int32_t ydot, int32_t zdot )
{
    ...
    double x_radians_per_sec = double(xdot) * TAU * (1/STEPPER_STEPS_PER_REV) * (1/PULSES_UNIT_CONVERSION); // 2pi radians per 200 counts
    double y_radians_per_sec = double(ydot) * TAU * (1/STEPPER_STEPS_PER_REV) * (1/PULSES_UNIT_CONVERSION); // 2pi radians per 200 counts
    double z_radians_per_sec = double(zdot) * TAU * (1/STEPPER_STEPS_PER_REV) * (1/PULSES_UNIT_CONVERSION); // 2pi radians per 200 counts
...
telemetry_t Controller(telemetry_t t, double dt_seconds)
{
    ...
    Matrix3d Proj_operator = ( Matrix3d::Identity() - (g_B*g_B.transpose()) ) / (g_B.norm()*g_B.norm());
    ...
    controller_output.r_mass_commanded = mm_mass_matrix.inverse() * (skew(g_B) * controller_output.u_com / (g_B.norm()*g_B.norm()) ); // desired commanded mass positions
