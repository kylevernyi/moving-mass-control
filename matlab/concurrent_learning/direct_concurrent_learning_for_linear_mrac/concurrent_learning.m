...
CL_point_accept_epsilon = 0.01; % how willing we are to accept new CL points
...
CL_Kr_gain = 0.1 / p_bar * inv(gamma_R);
...
R_cov = 0.0001; % measurement noise  covariance
Q_cov = 0.01*eye(2); % process noise covariance (turn this on or off in the dynamics, but good to keep some in the filter)
v_smooth = 10; % number of future measurements to consider while smoothing point j
...
legend(["$\hat x_1$" "$\hat x_2$" "$x_1$" "$x_2$"])
