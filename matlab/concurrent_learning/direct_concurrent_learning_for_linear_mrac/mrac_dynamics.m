...
function [xdot_return] = mrac_dynamics(t, x, A, B, Ar, Br, P, ...
    ...
    persistent X R Phi_stack Theta_hat_stack; % CL variables
    persistent x_last_point_stored p current_index_for_cyclic_stack; % CL variables
    persistent nu  Pi Sigma lambda j_smooth iter xdot_hat_available ; %  Kalman filter variables
    persistent r;
    ...
    if isempty(iter)
        ...
        P_kf = [1*eye(1), zeros(1,1); 
                 zeros(1,1),  1e6*eye(1)]; % initial covariance matrix 
    end
...
if integration_phase
    y = H*nu(:,iter) + sqrtm(R_cov)*randn(1,1); % measurement
    ...
    %% Control
    % e = nu(:,iter) - [0;0]; % tracking error signal
    % e = x - xr;
        ...
        if (xdot_hat_available)
            ...
            if (  ( norm(nu(:,iter) - x_last_point_stored)^2 / norm(nu(:,iter)) >= CL_point_accept_epsilon)  || (rank([X, nu(:,iter)]) > rank(X))) 
                if (p < p_bar) % Initilization, record more data until p_bar points
                    ...
                    % [R, current_index_for_cyclic_stack] = updateHistoryStack(R, current_index_for_cyclic_stack, r, p_bar);
                    % point_added = [point_added t];
                    ...
                    if (max_S > S_min_old) % if any new combinations increase our minimum SV
                        ...
                        R(:,max_S_idx) = u;
                        Phi_stack(:,max_S_idx) = Phi;
                        Theta_hat_stack(:,max_S_idx) = theta_hat;
                        x_last_point_stored = nu(:,iter);
                        point_added = [point_added t];
                    end
                end 
    ...
    concurrent_learning_Delta = 0;  % initialize sum to zero
    epsilon_Delta = zeros([p 1]);
    for j = 1:p
        x_j = X(1,j); % jth state vector from storage
        xdot_j = X(2, j);
        u_j = R(:,j);
        Phi_j = Phi_stack(:, j);
        Delta_j = xdot_j - x_j - u_j;
        
        theta_hat_j = Theta_hat_stack(:,j);

        epsilon_Delta(j, :) = Phi_j*theta_hat_j - Delta_j;
    ...
    xdot = x + Phi*Theta + u; %+ sqrtm(Q_cov)*randn(2,1)*0;
    % xr_dot = Ar*xr + Br*r;

    theta_hat_dot = gamma_x*(Phi*x + CL_on * concurrent_learning_Delta);
       
    % V = 0.5*e'*P*e + 0.5*trace(K_tilde'*inv(gamma_x)*K_tilde) + 0.5*trace(Kr_tilde'*inv(gamma_R)*Kr_tilde); % lyap function
    V = 0.5*x'*P*x + 0.5*theta_tilde^2;
    % xdot_return = [xdot; xr_dot; K_dot; Kr_dot];
    xdot_return = [xdot; theta_hat_dot];
