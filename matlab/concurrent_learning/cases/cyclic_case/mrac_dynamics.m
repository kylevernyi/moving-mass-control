...
function [xdot_return, U_return] = mrac_dynamics(t, x, A, B, Ar, Br, P, ...
    ...
    if (  ( norm(x - x_last_point_stored)^2 / norm(x) >= epsilon)  || (rank([X, x]) > rank(X))) 
        ...
        % if p >= p_bar % Now we only add new points if it makes our minimum singular value (msv) increase
        ...
        %         X = T; % restore X back to its original value for the next run
        %     end
        % 
        ...
        %         p = p-1;
        %         X = T;
        %     end % minimum increase check end
    ...
    for j = 1:p
        ...
        e_K(j) = inv(B'*B)*B' * (xdot_j  - Ar*x_j - Br*r_j - B*e_Kr(j)); % wrong ?   inv(B'*B) * B' same as pinv

        concurrent_learning_x = concurrent_learning_x + x_j*e_K(j)'; % wrong ?
        concurrent_learning_R = concurrent_learning_R + r_j*e_Kr(j)'; % good 
    end
    ...
    U_return = [u,  r,  min(svd(X',"vector")), u_pd, u_rm, V];
