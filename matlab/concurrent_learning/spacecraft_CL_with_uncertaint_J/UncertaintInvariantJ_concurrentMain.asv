...
clear; clc; close all; 
clear all ; clear classes; clear uncertainInvariantJ_concurrentDynamics;
...
Theta = [0.025; 0.045; 0.066]; %Roff Vector from Center of Rotation to Center of Mass
m = 4.2; % mass in kg
% Reference model

poles = 2*[-0.1+0.1i, -0.1-0.1i, -0.15*sqrt(2)+0.1i, -0.15*sqrt(2)-0.1i, -0.15+0.15i, -0.15-0.15i ];
K = -place(A, B, poles);
Ar = A + B*K;
...
dt = 0.001;  % Time step
tf = 25;     % Total simulation time
t = 0:dt:tf;   % Time vector

%% Initial Conditions
% sigma = [0.1; -0.1; 0.1]; 
% sigma_dot = [0.01; 0.01; 0.01];
% x = [sigma; sigma_dot];
x = zeros(6,1);
xr = zeros(6,1);
theta_hat = zeros(3,1);
...
dynamicsFunction = @(t,x) uncertainInvariantJ_concurrentDynamics(t, x, A, B, Ar, Br, J, m, Theta, K, Kr, P, Gamma, ...
    adaptive_on, CL_on, p_bar, CL_point_accept_epsilon, CL_gain, dt, true);
backsolvingFunction = @(t,x) uncertainInvariantJ_concurrentDynamics(t, x, A, B, Ar, Br, J, m, Theta, K, Kr, P, Gamma, ...
