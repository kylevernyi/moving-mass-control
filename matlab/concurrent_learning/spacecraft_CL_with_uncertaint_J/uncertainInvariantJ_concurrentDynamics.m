function [dxdt, U] = uncertainInvariantJ_concurrentDynamics(t, x, A, B, Ar, Br, J, m, Theta, K, Kr, P, Gamma, ...
...
if (integration_phase  && (mod(rk4_counter, 4)==0) )  % only do this if integrating not backsolving since global vars get messed up 
    ...
    for j = 1:p
        ...
        epsilon_Delta(:,j) = Phi_j*theta_hat - Delta_j;
  ...
  % Record the MinSVD
    if (integration_phase && (mod(rk4_counter, 4)==0) )
